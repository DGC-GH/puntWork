...
(function($, window, document) {
    ...
    var JobImportAPI = {
         ...
         * @returns {Promise} AJAX promise
         */
        runImportDiagnostics: function() {
            console.log('[PUNTWORK] ===== API.runImportDiagnostics DEBUG =====');
            console.log('[PUNTWORK] Running comprehensive import diagnostics');
            console.log('[PUNTWORK] AJAX URL:', jobImportData.ajaxurl);
            console.log('[PUNTWORK] Nonce:', jobImportData.nonce);
            console.log('[PUNTWORK] Timestamp:', new Date().toISOString());
            ...
            return $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'run_import_diagnostics',
                    nonce: jobImportData.nonce
                },
                timeout: 30000, // 30 seconds for diagnostics
                success: function(response) {
                    console.log('[PUNTWORK] AJAX success for runImportDiagnostics:', response);
                    if (!response || !response.success) {
                        console.error('[PUNTWORK] ERROR: runImportDiagnostics AJAX returned unsuccessful response:', response);
                    } else {
                        console.log('[PUNTWORK] SUCCESS: Diagnostics completed');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('[PUNTWORK] AJAX error for runImportDiagnostics:', {
                        xhr: xhr,
                        status: status,
                        error: error,
                        statusCode: xhr.status,
         ...
         * @param {number} jobId - The Action Scheduler job ID to run
         * @returns {Promise} AJAX promise
         */
        forceRunBatchJob: function(jobId) {
            console.log('[PUNTWORK] ===== API.forceRunBatchJob DEBUG =====');
            console.log('[PUNTWORK] Forcing run of batch job ID:', jobId);
            console.log('[PUNTWORK] AJAX URL:', jobImportData.ajaxurl);
            console.log('[PUNTWORK] Nonce:', jobImportData.nonce);
            console.log('[PUNTWORK] Timestamp:', new Date().toISOString());
            ...
            return $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'force_run_batch_job',
                    job_id: jobId,
                    nonce: jobImportData.nonce
                },
                timeout: 60000, // 60 seconds for job execution
                success: function(response) {
                    console.log('[PUNTWORK] AJAX success for forceRunBatchJob:', response);
                    if (!response || !response.success) {
                        console.error('[PUNTWORK] ERROR: forceRunBatchJob AJAX returned unsuccessful response:', response);
                    } else {
                        console.log('[PUNTWORK] SUCCESS: Batch job executed');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('[PUNTWORK] AJAX error for forceRunBatchJob:', {
                        xhr: xhr,
                        status: status,
                        error: error,
                        statusCode: xhr.status,
