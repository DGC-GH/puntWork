...
class PuntWorkAPITestSuite {
	...
	private function testWordPressConnectivity() {
		...
		$result  = $this->makeRequest( '/wp-json/', 'GET', null, array(), $this->wpRootUrl );
	...
	private function testPluginActivation() {
		...
		$result       = $this->makeRequest( '/wp-json/puntwork/v1/import-status?api_key=invalid', 'GET', null, array(), $this->wpRootUrl );
	...
	private function testTriggerImportEndpoint() {
		...
		if ( $statusResult['http_code'] === 200 && isset( $statusResponse['success'] ) && $statusResponse['success'] ) {
			$status        = $statusResponse['status'] ?? array();
		...
		$result = $this->makeRequest( '/trigger-import', 'POST', $data, array(), null, 30 ); // Initial trigger timeout
		...
		if ( $testModeSuccess ) {
			...
			$this->logTest(
				'Trigger Import - Concurrent Prevention',
				$concurrentRejected,
				array(
					'http_code' => $result['http_code'],
					'response'  => $this->parseJsonResponse( $result['response'] ),
	...
	private function testErrorHandling() {
		...
		// Test malformed JSON
		$result           = $this->makeRequest( '/trigger-import', 'POST', 'invalid json', array( 'Content-Type: application/json' ) );
	 ...
	 * Make HTTP request
	 */
	private function makeRequest( $endpoint, $method = 'GET', $data = null, $headers = array(), $customBaseUrl = null, $timeout = 30 ) {
	 ...
	 * Log test result
	 */
	private function logTest( $testName, $success, $details = array() ) {
	...
	private function checkApiKeyConfigured() {
		$result = $this->makeRequest( '/wp-json/puntwork/v1/import-status?api_key=' . $this->apiKey, 'GET', null, array(), $this->wpRootUrl );
