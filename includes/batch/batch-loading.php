...
/**
 * Count total items in JSONL file.
 *
 * @param  string $json_path Path to JSONL file.
...
function get_json_item_count( $json_path ) {
	...
	if ( $debug_mode ) {
		...
		error_log( '[PUNTWORK] [COUNT-START] Counting items in: ' . $json_path );
		error_log( '[PUNTWORK] [COUNT-START] File exists: ' . ( file_exists( $json_path ) ? 'yes' : 'no' ) );
		error_log( '[PUNTWORK] [COUNT-START] File readable: ' . ( is_readable( $json_path ) ? 'yes' : 'no' ) );
		if ( file_exists( $json_path ) ) {
			error_log( '[PUNTWORK] [COUNT-START] File size: ' . filesize( $json_path ) . ' bytes' );
		}
	}

	if ( ! file_exists( $json_path ) || ! is_readable( $json_path ) ) {
		if ( $debug_mode ) {
			error_log( '[PUNTWORK] [COUNT-ERROR] File not accessible: ' . $json_path );
		}
		return 0;
	}
	...
	if ( ( $handle = fopen( $json_path, 'r' ) ) === false ) {
		if ( $debug_mode ) {
			error_log( '[PUNTWORK] [COUNT-ERROR] Cannot open file for counting: ' . $json_path );
		}
		return 0;
	}

	// For large files, sample a portion to estimate total count
	$file_size = filesize( $json_path );
	$sample_size = 1000; // Check first 1000 lines for average line size
	$estimated_total = 0;

	if ( $file_size > 1000000 ) { // If file > 1MB, use sampling
		$sample_lines = 0;
		$total_chars = 0;

		while ( $sample_lines < $sample_size && ( $line = fgets( $handle ) ) !== false ) {
			$line = trim( $line );
			if ( ! empty( $line ) ) {
				// Remove BOM if present
				if ( substr( $line, 0, 3 ) === $bom ) {
		...
		if ( $sample_lines > 0 ) {
			$avg_line_size = $total_chars / $sample_lines;
			$estimated_total = (int) ( $file_size / $avg_line_size );

			if ( $debug_mode ) {
				error_log( '[PUNTWORK] [COUNT-SAMPLE] Sampled ' . $sample_lines . ' lines, avg size: ' . round( $avg_line_size, 2 ) . ' chars, estimated total: ' . $estimated_total );
			}
		}

		// Reset file pointer for full count if file is reasonably sized
		if ( $estimated_total < 50000 ) { // Less than 50k items
			rewind( $handle );
			$count = 0;
		} else {
			// Use estimate for very large files
			fclose( $handle );
			if ( $debug_mode ) {
				error_log( '[PUNTWORK] [COUNT-END] Using estimate for large file: ' . $estimated_total . ' items' );
	...
	while ( ( $line = fgets( $handle ) ) !== false ) {
		...
		if ( ! empty( $line ) ) {
			...
			if ( $item !== null ) {
				...
				error_log( '[PUNTWORK] [COUNT-WARN] Invalid JSON line skipped: ' . substr( $line, 0, 100 ) . '...' );
			}
		}
	}
	...
	if ( $debug_mode ) {
		error_log( '[PUNTWORK] [COUNT-END] Counted ' . $count . ' valid JSON items in file' );
		error_log( '[PUNTWORK] [COUNT-END] ===== GET_JSON_ITEM_COUNT END =====' );
	}
