...
function load_and_prepare_batch_items( string $json_path, int $start_index, int $batch_size, float $threshold, array &$logs ): array {
	...
	if ( $debug_mode ) {
		error_log( '[PUNTWORK] [LOAD-DEBUG] About to call load_json_batch_streaming with parameters:' );
...
function load_json_batch_streaming( $json_path, $start_index, $batch_size ) {
	...
	$debug_mode = defined( 'WP_DEBUG' ) && WP_DEBUG;
	// Always log critical information for debugging import issues
	error_log( '[PUNTWORK] load_json_batch_streaming: ===== LOAD_JSON_BATCH_STREAMING START =====' );
	error_log( '[PUNTWORK] load_json_batch_streaming: called with start_index=' . $start_index . ', batch_size=' . $batch_size . ', json_path=' . $json_path );
	error_log( '[PUNTWORK] load_json_batch_streaming: file_exists=' . (file_exists($json_path) ? 'yes' : 'no') . ', is_readable=' . (is_readable($json_path) ? 'yes' : 'no') . ', filesize=' . (file_exists($json_path) ? filesize($json_path) : 'N/A') );
	error_log( '[PUNTWORK] load_json_batch_streaming: get_json_item_count=' . get_json_item_count($json_path) );
	if ( $debug_mode ) {
		error_log( '[PUNTWORK] load_json_batch_streaming: WP_DEBUG is enabled' );
	} else {
		error_log( '[PUNTWORK] load_json_batch_streaming: WP_DEBUG is disabled' );
	}
	...
	if ( ( $handle = fopen( $json_path, 'r' ) ) === false ) {
		error_log( '[PUNTWORK] load_json_batch_streaming: CRITICAL - Cannot open file: ' . $json_path . ', error: ' . error_get_last()['message'] ?? 'unknown' );
		return array( 'items' => $items, 'lines_read' => $lines_read );
	}

	error_log( '[PUNTWORK] load_json_batch_streaming: File opened successfully, handle is valid: ' . (is_resource($handle) ? 'yes' : 'no') );

	// Skip to start_index with memory-efficient line skipping
	$current_line = 0;
	$bom = "\xef\xbb\xbf";
	$skipped_lines = 0;
	while ( $current_line < $start_index && ( $line = fgets( $handle ) ) !== false ) {
		...
		$skipped_lines++;
		if ( $debug_mode && $skipped_lines <= 5 ) {
			error_log( '[PUNTWORK] load_json_batch_streaming: Skipping line ' . $current_line . ', length=' . strlen($line) );
		}
		// Free memory immediately after processing each skipped line
		unset( $line );
	}

	error_log( '[PUNTWORK] load_json_batch_streaming: Skipped ' . $skipped_lines . ' lines to reach start_index=' . $start_index . ', current position at line ' . $current_line );

	// Read batch_size items with memory management
	$items_read = 0;
	$empty_lines = 0;
	$invalid_json_lines = 0;
	$total_lines_attempted = 0;
	...
	while ( $items_read < $batch_size && ( $line = fgets( $handle ) ) !== false ) {
		...
		if ( $debug_mode && $total_lines_attempted <= 3 ) {
			error_log( '[PUNTWORK] load_json_batch_streaming: Reading line ' . $current_line . ', original length=' . strlen($original_line) . ', trimmed length=' . strlen($line) );
		}
		...
		if ( substr( $line, 0, 3 ) === $bom ) {
			$line = substr( $line, 3 );
			if ( $debug_mode && $total_lines_attempted <= 3 ) {
				error_log( '[PUNTWORK] load_json_batch_streaming: Removed BOM from line ' . $current_line );
			}
		}
		...
		if ( empty( $line ) ) {
			$empty_lines++;
			if ( $debug_mode && $empty_lines <= 3 ) {
				error_log( '[PUNTWORK] load_json_batch_streaming: Empty line at ' . $current_line );
			}
			unset( $line, $original_line );
			continue;
		}
		...
		if ( $item !== null ) {
			...
			$items_read++;
			if ( $debug_mode && $items_read <= 3 ) {
				error_log( '[PUNTWORK] load_json_batch_streaming: Successfully decoded item ' . $items_read . ' at line ' . $current_line . ', GUID=' . ($item['guid'] ?? 'MISSING') );
			}
		} else {
			$invalid_json_lines++;
			if ( $debug_mode && $invalid_json_lines <= 3 ) {
				error_log( '[PUNTWORK] load_json_batch_streaming: Invalid JSON at line ' . $current_line . ': ' . json_last_error_msg() . ', preview: ' . substr($line, 0, 100) );
			}
		}

		// Free memory immediately after processing each line
		unset( $line, $original_line );

		// Memory check - if we're approaching memory limits, stop early
		if ( memory_get_usage( true ) > get_memory_limit_bytes() * 0.8 ) {
			error_log( '[PUNTWORK] load_json_batch_streaming: MEMORY LIMIT APPROACHING - stopping batch early at ' . $items_read . ' items' );
			break;
		}

		// Safety check to prevent infinite loops
		if ( $total_lines_attempted > $batch_size * 10 ) {
			error_log( '[PUNTWORK] load_json_batch_streaming: SAFETY BREAK - attempted ' . $total_lines_attempted . ' lines, breaking to prevent infinite loop' );
			if ( $debug_mode ) {
				error_log( '[PUNTWORK] load_json_batch_streaming: SAFETY BREAK - attempted ' . $total_lines_attempted . ' lines, breaking to prevent infinite loop' );
			}
			break;
	...
	error_log( '[PUNTWORK] load_json_batch_streaming: Read loop completed - items_read=' . $items_read . ', lines_read=' . $lines_read . ', total_attempted=' . $total_lines_attempted . ', empty=' . $empty_lines . ', invalid=' . $invalid_json_lines );

	fclose( $handle );

	if ( $debug_mode ) {
		error_log( '[PUNTWORK] load_json_batch_streaming: returning ' . count( $items ) . ' items (read ' . $lines_read . ' lines, attempted ' . $total_lines_attempted . ' lines, empty=' . $empty_lines . ', invalid=' . $invalid_json_lines . ')' );
		if ( count( $items ) == 0 && $lines_read > 0 ) {
			error_log( '[PUNTWORK] load_json_batch_streaming: CRITICAL - Read ' . $lines_read . ' lines but got 0 valid items! This indicates file corruption or format issues.' );
		}
		error_log( '[PUNTWORK] load_json_batch_streaming: ===== LOAD_JSON_BATCH_STREAMING END =====' );
	}

	// Always log the result for debugging
	error_log( '[PUNTWORK] load_json_batch_streaming: RESULT - returning ' . count( $items ) . ' items (lines_read=' . $lines_read . ', attempted=' . $total_lines_attempted . ', empty=' . $empty_lines . ', invalid=' . $invalid_json_lines . ')' );
	if ( count( $items ) == 0 ) {
		error_log( '[PUNTWORK] load_json_batch_streaming: CRITICAL - Returning 0 items! This is the root cause of the import failure.' );
	}
