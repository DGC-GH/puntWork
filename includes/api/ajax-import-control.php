...
function test_single_job_import_ajax()
{
    ...
    try {
        ...
        PuntWorkLogger::info('Starting test single job import', PuntWorkLogger::CONTEXT_AJAX, [
            'guid' => $test_job['guid'],
        ...
        if (!empty($existing_post)) {
            PuntWorkLogger::warn('Test job already exists', PuntWorkLogger::CONTEXT_AJAX, [
                'guid' => $test_job['guid'],
        ...
        if (is_wp_error($post_id)) {
            PuntWorkLogger::error('Failed to create test job post', PuntWorkLogger::CONTEXT_AJAX, [
        ...
        PuntWorkLogger::info('Test job post created', PuntWorkLogger::CONTEXT_AJAX, [
            'post_id' => $post_id,
        ...
        if (!$verify_post) {
            PuntWorkLogger::error('Test job creation verification failed', PuntWorkLogger::CONTEXT_AJAX, [
                'post_id' => $post_id
            ]);
        ...
        if (empty($final_check)) {
            PuntWorkLogger::error('Final verification failed - job not found in database', PuntWorkLogger::CONTEXT_AJAX, [
                'post_id' => $post_id
            ]);
        ...
        PuntWorkLogger::info('Test single job import completed with full verification', PuntWorkLogger::CONTEXT_AJAX, [
            'post_id' => $post_id,
        ...
        PuntWorkLogger::logAjaxResponse('test_single_job_import', [
            'post_id' => $post_id,
