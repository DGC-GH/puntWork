...
function ajax_manual_feed_import() {
    log_import_event( "AJAX: Manual import STARTED for feed ID {$_POST['feed_id'] ?? 'unknown'} by user " . get_current_user_id(), 'info' );
    ...
    if ( ! $post_id ) {
        log_import_event( 'AJAX: Invalid feed ID provided: ' . ($post_id ?? 'none'), 'error' );
