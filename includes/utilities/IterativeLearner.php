...
class IterativeLearner {
	 ...
	 */
	private static array $debug_info = array();

	/**
	 * Learning configuration.
	 */
	private static array $learning_config = array(
		'enabled'              => true,
	 ...
	 */
	public static function recordSessionPerformance( array $session_data, array $optimization_context = array() ): bool {
		if ( ! self::$learning_config['enabled'] ) {
			return true;
		}

		try {
			$performance_record = array(
		...
		foreach ( $recent_sessions as $index => $session ) {
			...
			foreach ( $weights_used as $strategy => $weight ) {
				if ( ! isset( $analysis['strategy_effectiveness'][ $strategy ] ) ) {
					$analysis['strategy_effectiveness'][ $strategy ] = array(
			...
			$analysis['performance_correlations'][] = array(
	...
	private static function applyWeightAdjustments( array $weight_adjustments ): array {
		...
		$adjustment_history   = get_option( self::WEIGHT_ADJUSTMENTS_KEY, array() );
	...
	private static function getPerformanceHistory(): array {
		return get_option( self::PERFORMANCE_DATA_KEY, array() );
	}
	...
	public static function adjustWeight( string $strategy, float $adjustment ): bool {
		$adjustments = array( $strategy => $adjustment );
