...
function schedule_async_batch_job( array $batch_data, int $priority = 10 ): ?string {
	...
	if ( is_action_scheduler_available() && function_exists( 'as_schedule_single_action' ) ) {
		...
		return as_schedule_single_action( time(), $hook, array( $batch_data ), 'puntwork', false, $priority );
...
function process_async_batch_job( array $batch_data ): array {
	try {
		...
		$batch_results              = get_option( 'puntwork_async_batch_results', array() );
		$batch_results[ $batch_id ] = array(
			'result'          => $result,
			'processed_range' => array( $start_index, $end_index ),
			'timestamp'       => time(),
		);
...
function aggregate_async_batch_results( array $job_ids ): array {
	...
	$filtered_results = array_filter(
		$batch_results,
		function ( $result ) use ( $cutoff_time ) {
...
function check_async_import_status(): array {
	$async_jobs    = get_option( 'puntwork_async_import_jobs', array() );
