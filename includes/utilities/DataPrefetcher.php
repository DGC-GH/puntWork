...
class DataPrefetcher {
	 ...
	 */
	private static array $prefetch_config = array(
		'post_metadata'  => array(
			'enabled'    => true,
			'batch_size' => 100,
		...
		'taxonomy_terms' => array(
			'enabled'    => true,
			'taxonomies' => array( 'category', 'post_tag', 'job_type', 'location' ),
	 ...
	 */
	public static function prefetchForBatch( array $batch_guids, array $batch_items = array() ): array {
	...
	private static function prefetchAcfFields(): array {
		...
		$prefetched_fields = array();

		foreach ( $field_groups as $group ) {
			$fields = acf_get_fields( $group['key'] );
			if ( $fields ) {
				$prefetched_fields[ $group['key'] ] = array(
					'group'  => $group,
					'fields' => $fields,
				);
	...
	private static function prefetchUserData(): array {
		...
		$prefetched_users = array();
		foreach ( $recent_users as $user ) {
			$prefetched_users[ $user->ID ] = array(
				'ID'           => $user->ID,
