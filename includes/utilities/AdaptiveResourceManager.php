...
class AdaptiveResourceManager {
	 ...
	 */
	private static array $resource_profiles = array(
		'small'  => array(
			'memory_limit'       => '256M',
	 ...
	 */
	private static array $performance_history = array();

	/**
	 * Current resource allocation state.
	 */
	private static array $current_allocation = array();
	...
	public static function analyzeAndAllocate( array $batch_data ): array {
		$batch_size             = $batch_data['batch_size'] ?? count( $batch_data['batch_items'] ?? array() );
	...
	private static function getNextProfile( string $current ): string {
		$order         = array( 'small', 'medium', 'large', 'xlarge' );
	...
	public static function recordPerformanceMetrics( array $metrics ): void {
		...
		$existing = EnhancedCacheManager::get( $history_key, CacheManager::GROUP_ANALYTICS ) ?: array(
			'avg_memory_mb'    => 0,
			'avg_time_seconds' => 0,
			'success_rate'     => 1.0,
			'sample_count'     => 0,
	...
	public static function getCurrentAllocation(): array {
		return self::$current_allocation ?: array( 'status' => 'no_allocation' );
	}
