...
class FeedOptimizer
{
    ...
    public static function init(): void
    {
        ...
        add_action('puntwork_feed_optimization', [__CLASS__, 'runScheduledOptimization']);

        // Add AJAX endpoint for manual optimization
        add_action('wp_ajax_run_feed_optimization', [__CLASS__, 'ajaxRunOptimization']);
    }
    ...
    public static function runScheduledOptimization(): void
    {
        try {
            ...
            update_option('puntwork_last_optimization', [
    ...
    {
        $frequencies = ['weekly' => 'daily', 'daily' => '12hours', '12hours' => '6hours', '6hours' => '4hours', '4hours' => '2hours', '2hours' => 'hourly', 'hourly' => '30min', '30min' => '15min', '15min' => '5min'];
        return $frequencies[$currentFrequency] ?? $currentFrequency;
    }
    ...
    public static function getOptimizationRecommendations(): array
    {
        ...
        try {
            ...
            foreach ($feeds as $feed) {
                ...
                if (!empty($feedRecs)) {
                    $recommendations['feed_optimizations'][] = [
                        'feed_id' => $feed->ID,
                        'feed_name' => $feed->post_title,
    ...
    private static function getStaleFeeds(): array
    {
        ...
        $stale_feeds = $wpdb->get_col($wpdb->prepare("
